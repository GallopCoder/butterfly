...
public class DupDetectUtil {
    ...
    public static DuplicateEntity dupTry(String ip, int port, String var0){
        ...
        try{
            ...
            byte bb[]=md.digest(var0.getBytes());
            //用该md5值去调用该服务
            DID did=dd.dupTry(bb,DupDetector.MODE_ALL_ID);//try模式：不会在dd中记录数据
...
//            DuplicateEntity aCheck = dupCheck("192.168.3.208", 7010, string);
//            System.out.println("[aCheck ]  " + aCheck);
            DuplicateEntity aTry = dupTry("192.168.3.208", 7010, string);
